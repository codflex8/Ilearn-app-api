...
class Websocket extends Server {
  ...
  public static async sendActiveRoomsToUsers() {
    ...
    // const usersRoom: Record<string, Array<GroupsChat>> = {};
    // activeRoomsIds.forEach((roomId) => {
    //   const users: User[] = this.rooms[roomId]?.users;
    //   const groupChat: GroupsChat = this.rooms[roomId]?.group;
    //   users.map((user) => {
    //     const userGroups = [...(usersRoom[user.id] ?? [])];
    //     if (!userGroups.find((g) => g.id === groupChat.id)) {
    //       userGroups.push(groupChat);
    //     }
    //     usersRoom[user.id] = userGroups;
    //   });
    // });
    // Object.entries(usersRoom).map(([userId, groupsChat]) => {
    //   const userSocketId = this.usersSockets[userId];
    //   this.io
    //     .to(userSocketId)
    //     .emit("active-rooms", { activegGroupsChat: groupsChat });
    // });
  }
}
